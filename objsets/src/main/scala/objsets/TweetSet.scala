...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("Empty Set. No tweets.")

  def mr(acc: Tweet): Tweet = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = this.mr(new Tweet("", "", -1))
  
  def mr(acc: Tweet): Tweet = {
    if (elem.retweets > acc.retweets) left.mr(right.mr(elem))
    else left.mr(right.mr(acc))
  }
